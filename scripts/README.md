...
- `release/`: local release artifact helpers. These may build local binaries
  ...
  compatibility symlink, service-account metadata, and systemd metadata without
  installing them, validate generated release artifacts, orchestrate the local
  release flow, stage a local proof-of-layout install tree under ignored `dist/`
  ...
  build native installers, modify services, grant sudo permissions, or execute
  package managers.
