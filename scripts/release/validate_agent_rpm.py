...
def validate_embedded_release_manifest(manifest_path: Path, buildroot: Path, version: str) -> None:
    ...
    if release_manifest.get("package_type") != "rpm-staging":
        raise ValueError("Embedded release manifest package_type must be rpm-staging.")
...
def validate_rpm(repo_root: Path, version: str, rpm_root: Path, reporter: Reporter) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate a local RPM staging tree for oaw-agent without building an RPM.")
...
def build_summary(reporter: Reporter, repo_root: Path, version: str, rpm_root: Path | None) -> dict[str, Any]:
    return {
...
def main() -> int:
    ...
    try:
        ...
        validate_rpm(repo_root, version, rpm_root, reporter)
    except Exception as exc:
        reporter.check("rpm staging validator", False, str(exc))

    summary = build_summary(reporter, repo_root, version, rpm_root)
