...
| CISA Known Exploited Vulnerabilities | Cybersecurity and Infrastructure Security Agency; [canonical catalog](https://www.cisa.gov/known-exploited-vulnerabilities-catalog), [official machine-readable mirror](https://github.com/cisagov/kev-data), [schema](https://raw.githubusercontent.com/cisagov/kev-data/develop/known_exploited_vulnerabilities_schema.json), and [CC0 1.0 license](https://github.com/cisagov/kev-data/blob/develop/LICENSE) | One-shot exact-path retrieval, strict normalization, local signing, static-mirror redistribution, operator-controlled import/activation/rollback, exact-CVE correlation, and prioritization of existing affected matches | `approved-with-obligations`; CC0 1.0 permits copying, modification, distribution, and commercial use without relicensing the data as Apache-2.0 | Preserve `CC0-1.0`, CISA identity, official-mirror/canonical-page distinction, catalog version/release/retrieval time, source and payload digests, adapter version, and OpenAssetWatch normalization notice. Treat every field as untrusted bounded text. Do not infer version applicability, severity, local exploitation, compromise, ransomware presence, or a local SLA; do not execute `requiredAction`. Corrections/removals and rollback retain history. No personal data is expected. The upstream JSON is HTTPS-delivered but becomes an OpenAssetWatch trusted artifact only after isolated publisher signing and normal verification/approval. Approved by project maintainer for this exact adapter scope on 2026-08-04; re-review by 2027-08-04 or immediately after ownership, license, schema, endpoint, or CISA terms change. |
...
signed index and artifacts. The separate CISA KEV approval permits only the
exact adapter and prioritization boundaries documented in `docs/CISA_KEV.md`;
it does not broaden the OSV approval or authorize other CISA sources. Other OSV
source families remain outside the mirror until their own decisions are
approved. Disabled source templates and publication controls are documented in
`docs/ADVISORY_MIRROR.md` and `docs/CISA_KEV.md`.
